...
(test graph-node-has-expected-properties
  ...
  (with-task-roots
    (let* ((g (build-task-graph))
      ...
      ;; No depot prefix — bare task IDs
      (is (not (find #\: sample-id)))
...
(test health-queries-return-lists
  ...
  (with-task-roots
    ;; Each should return a list (possibly empty)
...
;;; --- task-children Bare ID ---

(test task-children-returns-bare-ids
  "task-children returns bare (unqualified) child IDs."
  (with-task-roots
    (let* ((infos (get-cached-task-infos))
           (parent-with-children
             (loop for info in infos
                   for id = (getf info :id)
                   when (task-children id)
                   return id)))
      (when parent-with-children
        (let ((children (task-children parent-with-children)))
          ;; All children should be bare (no colon prefix)
          (dolist (child children)
            (is (not (find #\: child))
                "Child ~A of ~A should be bare (no depot prefix)"

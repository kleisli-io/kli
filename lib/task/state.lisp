  ...
  (let ((data (event-data event))
    ...
    (flet ((etag (element)
      ...
      (case (event-type event)
        ...
        (:task.spawn
         (let ((edge (encode-edge (strip-depot-prefix (getf data :child-id)) :phase-of)))
           (ors-add (task-state-edges state) edge (etag edge))))
        ;; Fork creates typed edge from parent to child
        (:task.fork
         (let ((edge (encode-edge (strip-depot-prefix (getf data :child-id))

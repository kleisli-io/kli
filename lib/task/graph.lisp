  ...
  (let ((dirs (directory (merge-pathnames "*/" (pathname (or tasks-root *tasks-root*)))))
    ...
    (dolist (dir dirs)
      (let* ((id (car (last (pathname-directory dir))))
             ...
             (latest-mod (max (if has-plan (file-mod-time (namestring plan-path)) 0)
                              (if has-events (file-mod-time (namestring events-path)) 0)
                              (if has-research (file-mod-time (namestring research-path)) 0))))
        (push (list :id id
...
(defun extract-token-overlap-edges (task-infos &key (min-overlap 2))
  "Extract edges between tasks sharing MIN-OVERLAP or more tokens.
   ...
   Uses inverted token index for O(N×K) instead of O(N²×M²)."
  (let ((task-tokens (make-hash-table :test 'equal))
    ...
    (dolist (info task-infos)
      (let* ((id (getf info :id))
             (tokens (remove-duplicates
                      (mapcar #'string-downcase (extract-task-tokens id))
                      :test #'equal)))
    ...
    (dolist (info task-infos)
      (let* ((id (getf info :id))
        ...
        (dolist (tok tokens)
          (dolist (other-id (gethash tok token-index))
            (unless (equal other-id id)
              (incf (gethash other-id candidates 0)))))
        (maphash (lambda (other-id count)
                   (when (>= count min-overlap)
                     (let ((key (if (string< id other-id)
                                    (cons id other-id)
                                    (cons other-id id))))
                       (unless (gethash key seen-pairs)
                         (setf (gethash key seen-pairs) t)
                         (push (list (car key) (cdr key) :topic) edges)))))
                 candidates)))
    edges))
...
(defun extract-same-day-edges (task-infos)
  "Layer 1 (inferred): tasks created on same day.
   Groups by date. Small groups (2-6) get full cliques.
  ...
  (let ((by-date (make-hash-table :test 'equal)))
    (dolist (info task-infos)
      (let ((date (getf info :date)))
        (when date
          (push (getf info :id) (gethash date by-date)))))
    (let ((edges nil))
...
(defun extract-all-state-edges (&optional (tasks-root *tasks-root*) infos)
  "Extract declared edges from all event-sourced tasks.
   ...
   When INFOS is provided, uses it to skip non-event tasks
   and avoid redundant directory scan."
  (unless tasks-root (detect-tasks-root))
  (let ((edges nil))
    (if infos
        ;; Fast path: use pre-scanned infos, skip tasks without events
        (dolist (info infos)
          (when (getf info :has-events)
            (let ((task-edges (extract-state-edges (getf info :id))))
              (dolist (edge task-edges)
                (let ((target (strip-depot-prefix (second edge))))
                  (push (list (first edge) target (third edge)) edges))))))
        ;; Fallback: scan directories (for standalone callers)
...
(defvar *date-prefix-scanner*
  (cl-ppcre:create-scanner "\\b(20[0-9]{2}-[0-9]{2}-[0-9]{2}-[a-zA-Z0-9][-a-zA-Z0-9]*)\\b")
  "Pre-compiled scanner for date-prefixed task ID patterns.
   Matches YYYY-MM-DD-<name> tokens.  Used by extract-reference-edges
   instead of building a per-build N-alternation regex (78K chars at N=1743).")

(defun extract-reference-edges (task-infos tasks-root)
  "Layer 2 (natural): cross-task citations in handoff documents.
   Uses a fixed date-prefix regex + hash-table lookup instead of building
   an N-alternation regex per build.  Hash-table dedup replaces pushnew O(M)."
  (let* ((id-set (make-hash-table :test 'equal))
         (edge-set (make-hash-table :test 'equal))
         (edges nil))
    (dolist (info task-infos)
      (setf (gethash (getf info :id) id-set) t))
    (dolist (info task-infos)
      (let* ((id (getf info :id))
        ...
        (dolist (hf handoffs)
          (handler-case
              (let ((content (uiop:read-file-string hf)))
                (cl-ppcre:do-matches-as-strings (match *date-prefix-scanner* content)
                  (when (and (gethash match id-set)
                             (not (equal match id)))
                    ;; O(1) dedup via canonical sorted key
                    (let ((key (if (string< id match)
                                   (cons id match)
                                   (cons match id))))
                      (unless (gethash key edge-set)
                        (setf (gethash key edge-set) t)
                        (push (list id match :reference) edges))))))
            (error () nil)))))
    edges))

(defun build-task-graph (&key (tasks-root *tasks-root*))
  "Build the task graph from a tasks root directory.
   Sole entry point for graph construction.
   Returns (values graph infos) so callers can reuse the infos list."
  (unless tasks-root (detect-tasks-root))
  (let* ((root (or tasks-root *tasks-root*))
    ...
    (dolist (edge (extract-reference-edges infos (namestring root)))
      (graph-add-edge graph (first edge) (second edge) (third edge)))
    (dolist (edge (extract-all-state-edges root infos))
      (graph-add-edge graph (first edge) (second edge) (third edge)))
    (values graph infos)))
...
(defvar *graph-cache-lock* (bt:make-recursive-lock "graph-cache")
  "Recursive mutex preventing concurrent graph builds (thundering herd).
   Recursive because get-cached-task-infos may call get-cached-task-graph
   while already holding the lock.")

(defvar *infos-cache* nil
  "Cached task infos from last graph build. Value: (timestamp . infos-list) or NIL.")

(defun get-cached-task-graph ()
  "Get cached task graph or build fresh. Uses TTL-based invalidation.
  ...
  (bt:with-recursive-lock-held (*graph-cache-lock*)
    (let* ((key "task-graph")
      ...
      (if valid
          (cdr entry)
          (multiple-value-bind (graph infos) (build-task-graph)
            ;; Reuse infos from build — no second scan-task-infos call.
...
;;; --- Task Infos Cache ---
;;; Caches the result of scan-task-infos so callers that only need
;;; task infos (not the full graph) avoid a redundant filesystem scan.
;;; Populated as a side-effect of get-cached-task-graph.
...
(defvar *task-state-cache-lock* (bt:make-lock "task-state-cache")
  "Protects *task-state-cache* from concurrent access.")

(defparameter *task-state-cache-ttl* 60
...
(defun invalidate-task-state-cache (task-id)
  "Invalidate cached state for a single task. Called from emit-event."
  (bt:with-lock-held (*task-state-cache-lock*)
    (remhash task-id *task-state-cache*)))

(defun clear-task-state-cache ()
  "Clear all cached task states."
  (bt:with-lock-held (*task-state-cache-lock*)
    (clrhash *task-state-cache*)))
...
;;; ============================================================
;;; Health Functions (Primary API)
;;; These call %find-* workers with *tasks-root*.
;;; ============================================================

(defun find-stale-tasks (&key (min-age-days 3))
  "Find stale tasks. Returns task IDs."
  (unless *tasks-root* (detect-tasks-root))
  (%find-stale-tasks *tasks-root* :min-age-days min-age-days))

(defun find-dead-ends (&key (min-idle-days 7))
  "Find dead-end tasks. Returns task IDs."
  (unless *tasks-root* (detect-tasks-root))
  (%find-dead-ends *tasks-root* :min-idle-days min-idle-days))

(defun find-declared-orphans ()
  "Find declared orphans. Returns task IDs."
  (unless *tasks-root* (detect-tasks-root))
  (%find-declared-orphans *tasks-root*))

(defun find-stale-claims (&key (timeout-hours 4))
  "Find stale claims. Returns task IDs."
  (unless *tasks-root* (detect-tasks-root))
  (%find-stale-claims *tasks-root* :timeout-hours timeout-hours))

(defun find-unexplored-frontier ()
  "Find unexplored frontier. Returns task IDs."
  (unless *tasks-root* (detect-tasks-root))
  (%find-unexplored-frontier *tasks-root*))

(defun find-convergent-clusters ()
  "Find convergent clusters."
  (unless *tasks-root* (detect-tasks-root))
  (%find-convergent-clusters *tasks-root*))

(defun find-premature-completions ()
  "Find premature completions."
  (unless *tasks-root* (detect-tasks-root))
  (%find-premature-completions *tasks-root*))

(defun find-unlinked-bidirectional-refs ()
  "Find unlinked bidirectional refs."
  (unless *tasks-root* (detect-tasks-root))
  (%find-unlinked-bidirectional-refs *tasks-root*))
                     ...
                     (loop for (a . rest) on ids
                           do (dolist (b rest)
                                (unless (or (gethash (cons a b) edge-pairs)
                                  ...
                                  (incf unlinked))))
                     (when (> unlinked 0)
                       (push (list topic ids unlinked) clusters)))))
               active-by-topic)
      ;; Graph-dependent checks: use cached graph
      (let* ((graph (get-cached-task-graph))
             (suggested (find-unlinked-bidirectional-refs-with-graph graph))
             ;; Markov + unorganized: use their own caching
             (missing (handler-case (find-missing-markov-edges)
                        (error () nil)))
             (unorganized (handler-case (find-unorganized-tasks)
                            (error () nil))))
        (list
         (cons :stale-forks (nreverse stale))
         (cons :dead-ends (nreverse dead))
         (cons :unlinked-roots (nreverse orphans))
         (cons :stale-claims (nreverse stale-claims))
         (cons :unexplored-frontier (nreverse frontier))
         (cons :convergent-clusters (sort clusters #'> :key #'third))
         (cons :premature-completions (nreverse premature))
         (cons :suggested-edges suggested)
         (cons :missing-edges missing)
         (cons :unorganized-tasks unorganized))))))
...
(defun task-children (task-id)
  "Return child task IDs for TASK-ID from edges OR-Set.
   ...
   Legacy depot:prefix IDs are stripped to bare form."
  (let ((events-path (task-events-path task-id)))
    (when (probe-file events-path)
      (handler-case
          (let* ((log (elog-load events-path))
              ...
              (let ((children nil))
                (dolist (type *child-bearing-edge-types*)
                  (dolist (target (edge-targets edges type))
                    (let ((bare (strip-depot-prefix target)))
                      (pushnew bare children :test #'string=))))
                (nreverse children))))
        (error (e)
...
(defun load-child-states (children)
  "Load CRDT states for a list of child task IDs. Returns hash-table id->state.
   Uses per-task state cache for O(1) hits on warm cache."

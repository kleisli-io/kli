  ...
  (let ((result nil))
    (dolist (entry (enrich-step (active)))
      (let* ((id (car entry))
        ...
        (unless (equal status "completed")
          (handler-case
              (let* ((state (task:get-cached-task-state id))
                     (claim (when state
                              (crdt:lww-value (task:task-state-claim state)))))
                ;; If task is claimed and claimer has departed
                (when (and claim (> (length claim) 0))
                  (let ((departure (task:session-has-left-p id claim)))
                    (when departure
                      (push entry result)))))
            (error () nil)))))
    result))
...
(defun enrich-step (node-set)
  ...
  (mapcar (lambda (entry)
            (let* ((id (car entry))
                   (props (cdr entry))
                   (state (task:get-cached-task-state id)))
              (if state
                  (handler-case
                      (let ((new-props (copy-list props)))
                        ...
                        ;; Markov enrichment fields (need raw events)
                        (handler-case
...
(defun safe-read-query (query-string)
  "Safely read a query string into an S-expression.
   ...
   Rejects non-string / empty input with a clear tq-parse-error
   instead of leaking SBCL's raw type-binding error."
  (cond
    ((null query-string)
     (error 'tq-parse-error
    ...
    ((zerop (length query-string))
     (error 'tq-parse-error :message "query is empty")))
  (let ((*read-eval* nil)
...
(defun max-existing-phase-number (task-id)
  ...
  (let ((children (task:task-children task-id))
    ...
    (dolist (child children max-n)
      (let* ((bare (task:strip-depot-prefix child))
             ;; Match "phase-N" or "YYYY-MM-DD-phase-N" patterns in bare ID
             (pos (search "phase-" bare :test #'char-equal)))
          (when pos
            (let* ((num-start (+ pos 6))

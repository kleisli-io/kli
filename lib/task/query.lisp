...
(defun get-plan-phases ()
  ...
  (let ((my-phases (follow (node *current-task-id*) :phase-of)))
    (if my-phases
        my-phases
        ;; No sub-phases - check if I'm a phase myself
        (let ((parents (back (node *current-task-id*) :phase-of)))
          (when parents
...
(defun validate-scaffold-plan-refs (phase-specs)
  ...
  (let ((defined-names (make-hash-table :test 'equal)))
    ...
    ;; Check all :after references
    (dolist (spec phase-specs t)
      (let ((after-pos (position :after spec)))
        (when after-pos
          (let ((deps (nthcdr (1+ after-pos) spec)))
            (dolist (dep deps)
...
(defun execute-scaffold-plan (phase-specs)
  "Execute (scaffold-plan! (p1 \"desc\") (p2 \"desc\" :after p1) ...).
   Two-pass: create all phases, then wire dependencies.
  ...
  (let ((result (make-scaffold-result :parent *current-task-id*))
    ...
    ;; Pass 1: Create all phases
    (dolist (spec phase-specs)
      (let* ((raw-name (symbol-name (first spec)))
             (description (second spec))
             ;; Auto-improve name from description if invalid
             (local-name (maybe-improve-name raw-name description))
             ;; Extract :after dependency (for pass 2)
             (after-pos (position :after spec))
        ...
        ;; Also map raw symbol name so :after references resolve
...
(defun check-enrich-required (node-set pred-form)
  "Check if pred-form uses enrich-required fields on non-enriched data.
   Signals tq-error with helpful message if so."
  (let ((fields (extract-pred-fields pred-form)))
    (when fields
      (let ((enrich-fields (intersection fields *enrich-required-fields*)))
        (when enrich-fields
          ;; Check if first node has enrich data
          (when (and node-set
                     (not (some (lambda (f) (getf (cdr (first node-set)) f))
                                *enrich-required-fields*)))
            (error 'tq-error
                   :message (format nil "Field~P ~{:~A~^, ~} require~:[s~;~] :enrich step first. ~
                                        Try: (-> ... :enrich (:where ...))"
                                    (length enrich-fields)
                                    (mapcar #'symbol-name enrich-fields)
                                    (> (length enrich-fields) 1)))))))))
...
(defun interpret-step (node-set step)
  ...
  (cond
    ...
    ((and (listp step) (eq (car step) :where))
     (check-enrich-required node-set (second step))
     (where-step node-set (interpret-where (second step))))
    ((and (listp step) (eq (car step) :select))
     (apply #'select-fields node-set (cdr step)))
    ((and (listp step) (eq (car step) :sort))
     (sort-by-field node-set (second step)))
    ((and (listp step) (eq (car step) :take))
     (take-n node-set (second step)))
    ((and (listp step) (eq (car step) :group-by))
     (group-by-step node-set (second step)))

...
(defun run-real-tasks (&rest roots)
  "Run benchmarks on real task event logs.
   ...
   Example: (run-real-tasks #P\"/path/to/depot1/.kli/tasks/\" #P\"/path/to/depot2/.kli/tasks/\")"
  (when (null roots)
    (error "run-real-tasks requires at least one directory root.~%~
            Usage: (run-real-tasks #P\"/path/to/.kli/tasks/\" ...)"))
...
;; Use (run-real-tasks #P"/path/to/.kli/tasks/" ...) directly,
...
(defun run-benchmarks (&key real-task-roots)
  "Run all benchmark suites and print combined results.
   ...
   Example: (run-benchmarks :real-task-roots '(#P\"/path/to/.kli/tasks/\"))"
  (let ((all-results nil))

...
(defun pq-activate-fn (query &key boost (top-k 5))
  "Activation wrapper for PQ (domain-agnostic).
   ...
   Emits :pattern.activate events to the task event stream."
      ...
      (let ((session-id (current-session-id)))
        (when session-id
          ...
          (let ((cwd (or (mcp-find-depot-root) (namestring (uiop:getcwd)))))
            ...
            (write-feedback-state-file cwd session-id))))
      ;; Emit :pattern.activate to task event stream (unified binary shares process)
      (let ((task-id task-mcp::*current-task-id*))
        (when (and task-id task:*tasks-root*)
          (handler-case
              (task:dashboard-emit-event
               task-id :pattern.activate
               (list :patterns pattern-ids
                     :query query
                     :session (or (current-session-id) "unknown")))
            (error (c)
              (format *error-output*
                      "~&[playbook] Failed to emit pattern.activate: ~A~%" c)))))
...
(mcp-framework:define-tool pq_query

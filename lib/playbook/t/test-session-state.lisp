;;;; playbook tests - Session State Management
...
(test get-or-create-session-creates-new
  ...
  (let ((playbook::*session-states* (make-hash-table :test 'equal)))
    (let ((session (get-or-create-session "test-new-001")))
      (is (not (null session)))
      (is (string= "test-new-001" (playbook::session-state-id session)))
      (is (null (playbook::session-state-activated-patterns session)))
      (is (null (playbook::session-state-feedback-given session)))
      (is (null (playbook::session-state-active-domains session))))))
...
(test record-activation-basic
  ...
  (let ((playbook::*session-states* (make-hash-table :test 'equal)))
    ...
    (let ((session (get-session "sess-act-001")))
      (is (= 2 (length (playbook::session-state-activated-patterns session))))
      (is (member "lisp-000001" (playbook::session-state-activated-patterns session)
                  :test #'string=)))))
...
(test record-activation-deduplicates
  ...
  (let ((playbook::*session-states* (make-hash-table :test 'equal)))
    ...
    (let ((session (get-session "sess-dedup-001")))
      (is (= 1 (length (playbook::session-state-activated-patterns session)))))))
  ...
  (let ((playbook::*session-states* (make-hash-table :test 'equal)))
    ...
    (let ((session (get-session "sess-fb-001")))
      ...
      (is (eq :helpful (cdr (assoc "lisp-000001"
                                   (playbook::session-state-feedback-given session)
                                   :test #'string=)))))))
...
(test record-domain-basic
  ...
  (let ((playbook::*session-states* (make-hash-table :test 'equal)))
    ...
    (let ((session (get-session "sess-dom-001")))
      (is (= 2 (length (playbook::session-state-active-domains session)))))))
...
(test record-domain-deduplicates
  ...
  (let ((playbook::*session-states* (make-hash-table :test 'equal)))
    ...
    (let ((session (get-session "sess-dom-dedup")))
      (is (= 1 (length (playbook::session-state-active-domains session)))))))
...
(test multiple-sessions-isolated
  ...
  (let ((playbook::*session-states* (make-hash-table :test 'equal)))
    ...
    (let ((a (get-session "session-A"))
      ...
      (is (equal '("pat-1") (playbook::session-state-activated-patterns a)))
      (is (equal '("pat-2") (playbook::session-state-activated-patterns b))))))

;;;; playbook tests - Relevance Feedback
...
(test relevance-record-and-roundtrip
  ...
  (let ((playbook::*relevance-store* (make-hash-table :test 'equal))
        (playbook::*relevance-store-lock* (bt:make-lock "test-relevance"))
    ...
    ;; Clear
    (bt:with-lock-held (playbook::*relevance-store-lock*)
      (clrhash playbook::*relevance-store*))
    (is (= 0 (hash-table-count playbook::*relevance-store*)))
    ;; Load
    (unwind-protect
         (let ((count (load-relevance-feedback-file path)))
           (is (= 2 count))
           (is (= 2 (length (gethash "nix-001" playbook::*relevance-store*))))
           (is (= 1 (length (gethash "lisp-042" playbook::*relevance-store*))))
           ;; Verify entry contents survived roundtrip
           (let ((entry (first (gethash "lisp-042" playbook::*relevance-store*))))
...
(test relevance-merge-dedup
  ...
  (let ((playbook::*relevance-store* (make-hash-table :test 'equal))
        (playbook::*relevance-store-lock* (bt:make-lock "test-merge"))
    ...
    (let ((existing-ts (relevance-entry-timestamp
                        (first (gethash "nix-001" playbook::*relevance-store*)))))
...
(test relevance-prune-removes-invalid
  ...
  (let ((playbook::*relevance-store* (make-hash-table :test 'equal))
        (playbook::*relevance-store-lock* (bt:make-lock "test-prune"))
    ...
    (unwind-protect
         (let ((pruned (prune-relevance-feedback path '("valid-001"))))
           (is (= 1 pruned))
           (is (= 1 (hash-table-count playbook::*relevance-store*)))
           (is-true (gethash "valid-001" playbook::*relevance-store*))
           (is-false (gethash "orphan-001" playbook::*relevance-store*)))
      (ignore-errors (delete-file path)))))
...
(test penalty-domain-overlap
  ...
  (let ((playbook::*relevance-store* (make-hash-table :test 'equal))
        (playbook::*relevance-store-lock* (bt:make-lock "test-penalty")))
    (record-relevance-feedback "nix-001" '("nix" "lisp"))
...
(test dispatch-normalize-feedback-type
  ...
  (is (eq :helpful (playbook::normalize-feedback-type :helpful)))
  (is (eq :harmful (playbook::normalize-feedback-type :harmful)))
  (is (eq :not-relevant (playbook::normalize-feedback-type :not-relevant)))
  (is (eq :not-relevant (playbook::normalize-feedback-type :irrelevant)))
  (is (eq :helpful (playbook::normalize-feedback-type "helpful")))
  (is (eq :harmful (playbook::normalize-feedback-type "harmful")))
  (is (eq :not-relevant (playbook::normalize-feedback-type "not-relevant")))
  (is (eq :not-relevant (playbook::normalize-feedback-type "irrelevant")))
  (is (null (playbook::normalize-feedback-type :bogus))))
...
(test scoring-penalty-reduces-sim
  ...
  (let ((playbook::*relevance-store* (make-hash-table :test 'equal))
        (playbook::*relevance-store-lock* (bt:make-lock "test-scoring")))

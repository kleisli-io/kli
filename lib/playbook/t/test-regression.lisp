...
;;; =========================================================================
;;; LIVE STATE SANITY CHECKS
;;; These test against the currently loaded playbook data to catch
;;; regressions where initialization or loading is broken.
;;; =========================================================================

(test live-patterns-loaded
  "Live daemon has patterns loaded."
  (is (> (pattern-count) 0)))

(test live-graph-has-edges
  "Live daemon has graph edges."
  (is (> (graph-edge-count) 0)))

(test live-embeddings-cached
  "Live daemon has embedding cache entries."
  (is (> (embedding-cache-size) 0)))

(test live-known-pattern-exists
  "A known pattern ID can be retrieved."
  (let ((p (get-pattern "lisp-000001")))
    (is (not (null p)))
    (when p
      (is (string= "lisp" (pattern-domain p)))
      (is (stringp (pattern-content p)))
      (is (> (length (pattern-content p)) 0)))))

(test live-graph-connectivity
  "Known pattern has graph edges."
  (let ((edges (graph-outgoing "lisp-000001")))
    (is (> (length edges) 0))))

(test live-activation-returns-results
  "Live activation produces non-empty results."
  (let ((results (playbook::activate-patterns-adaptive "lisp development" :top-k 3)))
    (is (> (length results) 0))
    (is (typep (car (first results)) 'playbook::pattern))))

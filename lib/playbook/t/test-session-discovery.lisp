;;;; playbook tests - Session Discovery
...
(test ensure-trailing-slash-adds-slash
  ...
  (is (string= "/foo/bar/" (playbook::ensure-trailing-slash "/foo/bar"))))
  ...
  (let* ((dir (format nil "/tmp/test-sess-disc-~a" (random 1000000)))
    ...
    (unwind-protect
         (is (string= "DEADBEEF-1234"
                       (playbook::read-latest-session-announcement dir)))
      (ignore-errors (delete-file path))
      (ignore-errors (uiop:delete-empty-directory claude-dir))
      (ignore-errors (uiop:delete-empty-directory dir)))))

(test read-latest-session-returns-nil-for-missing
  "read-latest-session-announcement returns NIL when .claude/latest-session is missing."
  (let* ((dir (format nil "/tmp/test-no-session-~a/" (random 1000000))))
    (ensure-directories-exist dir)
    (unwind-protect
         (let ((result (playbook::read-latest-session-announcement dir)))
           (is (null result)))
      (ignore-errors (uiop:delete-empty-directory dir)))))
...
(test read-latest-session-empty-file
  ...
  (let* ((dir (format nil "/tmp/test-sess-empty-~a" (random 1000000)))
    ...
    (with-open-file (s path :direction :output :if-exists :supersede)
      (write-string "" s))
    (unwind-protect
         (is (null (playbook::read-latest-session-announcement dir)))
      (ignore-errors (delete-file path))
      (ignore-errors (uiop:delete-empty-directory claude-dir))
      (ignore-errors (uiop:delete-empty-directory dir)))))
...
(test read-latest-session-missing-dir
  ...
  (is (null (playbook::read-latest-session-announcement
...
(test try-claim-session-creates-lock
  ...
  (let* ((dir (format nil "/tmp/test-claim-~a/" (random 1000000)))
         (playbook::*mcp-lock-path* nil))
    (ensure-directories-exist dir)
    (unwind-protect
         (progn
           (is-true (playbook::try-claim-session dir))
           (is (not (null playbook::*mcp-lock-path*)))
...
(test try-claim-session-fails-if-claimed
  ...
  (let* ((dir (format nil "/tmp/test-claim-fail-~a/" (random 1000000)))
    ...
    (unwind-protect
         (is-false (playbook::try-claim-session dir))
...
(test cleanup-mcp-lock-removes-file
  ...
  (let* ((dir (format nil "/tmp/test-cleanup-lock-~a/" (random 1000000)))
    ...
    (with-open-file (s lock-path :direction :output :if-exists :supersede)
      (format s "~A" (sb-posix:getpid)))
    (let ((playbook::*mcp-lock-path* lock-path))
      (cleanup-mcp-lock)
      (is (null playbook::*mcp-lock-path*))
...
(test cleanup-mcp-lock-noop-when-nil
  ...
  (let ((playbook::*mcp-lock-path* nil))

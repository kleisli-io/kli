  ...
  (multiple-value-bind (context protocol) (provider-baseline-context)
    ...
    (with-temp-credentials (path)
	  ...
	  (let ((gpt (models:find-model-definition registry "openai" "gpt-5.1")))
        ...
        (is (not (null (models:find-model-definition registry "openai" "gpt-5.6"))))
        (is (not (null (models:find-model-definition registry "openai" "gpt-5.6-sol"))))
        (is (not (null (models:find-model-definition registry "openai" "gpt-5.6-terra"))))
        (is (not (null (models:find-model-definition registry "openai" "gpt-5.6-luna"))))
        (is (null (models:find-model-definition registry "openai" "gpt-5.4")))
        ...
        (is (= 7 (count-provider-available "openai" registry store context)))))))

...
(defun register-pid-with-task-mcp (session-id pid &key team-env)
  ...
  (let ((session-id (jref input "session_id"))
    ...
    (let ((coord-root (depot:coordination-root-from cwd)))
      ...
      (let ((ollama-status (ensure-ollama)))
        ...
        ;; Detect agent team membership
        (let ((team-env (agent-team-env)))

          ;; Register PID with unified task-mcp daemon (includes team metadata).

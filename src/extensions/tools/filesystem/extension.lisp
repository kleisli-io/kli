...
(defextension read-tool
  (:provides
   (tool read
     :label "Read"
     :description "Read a file. Each output line is prefixed with its anchor LINE:HH where HH is a 2-hex-digit hash of the line's content, and edits reference these anchors. Optional start/end select an inclusive 1-based line range (end past EOF clamps). Set raw to true for unanchored content."
...
(defextension search-tool
    (:provides
     (tool search
           :label "Search"
           :description "Search file contents with a regular expression. Path is a single file or a glob, for example src/**/*.lisp. Each match prints as *LINE:HH content and context lines as space-prefixed LINE:HH content. These anchors are edit-ready - matched files need no read before editing. Optional context selects how many surrounding lines to show."
           :parameters '(:object (:pattern :string)
                                 (:path :string)
...
(defextension edit-tool
  (:provides
   (tool edit
     :label "Edit"
     :description "Apply a hashline patch. Read a file first — each line arrives as LINE:HH content, and edits reference these anchors.

...
(defun thinking-key (turn-id content-index)
  (list :thinking turn-id content-index))

(defun thinking-raw-seen-key (turn-id content-index)
  (list :thinking-raw-seen turn-id content-index))

(defun thinking-keys-for-turn (buffer turn-id)
  (loop for key being the hash-keys of buffer
        when (and (consp key)
                  (member (first key) '(:thinking :thinking-raw-seen))
                  (equal (second key) turn-id))
          collect key))
...
(defun project-agent-thinking-delta (event-type event mode-id buffer)
  ...
  (let* ((payload (kli/event:event-payload event))
         (turn-id (getf payload :turn-id))
         (content-index (getf payload :content-index))
         (text (or (getf payload :text) ""))
         (replacement-p (getf payload :replacement-p))
         (source (getf payload :source))
         (level (getf payload :level)))
    (when turn-id
      (let* ((key (thinking-key turn-id content-index))
             (raw-key (thinking-raw-seen-key turn-id content-index))
             (existing (gethash key buffer))
             (raw-p (eq source :raw))
...
(defun project-assistant-message-end (event-type event mode-id buffer)
  ...
  (let* ((payload (kli/event:event-payload event))
    ...
    (when turn-id
      ...
      (dolist (key (cons (delta-key turn-id)
                         (thinking-keys-for-turn buffer turn-id)))

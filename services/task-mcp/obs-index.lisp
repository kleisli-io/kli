...
(defun obs-graph-query (graph query-embedding &key (k 10))
  "Find k nearest observations to query embedding.
   Uses heap-select: maintains a min-heap of size K, avoiding full O(N log N) sort.
   Returns list of (text-hash . similarity) pairs, sorted descending."
  (bt:with-lock-held ((obs-graph-lock graph))
    (let ((heap (make-array k :fill-pointer 0))  ; min-heap by similarity
          (heap-min most-negative-double-float))
      ;; Scan all embeddings, maintaining top-K in min-heap
      (maphash (lambda (hash emb)
                 (let ((sim (vec-dot query-embedding emb)))
                   (cond
                     ;; Heap not full: always insert
                     ((< (fill-pointer heap) k)
                      (vector-push (cons hash sim) heap)
                      ;; After filling, compute min for fast rejection
                      (when (= (fill-pointer heap) k)
                        (setf heap-min (loop for i below k
                                             minimize (cdr (aref heap i))))))
                     ;; Heap full: only insert if better than worst
                     ((> sim heap-min)
                      ;; Find and replace the min element
                      (let ((min-idx 0)
                            (min-val (cdr (aref heap 0))))
                        (loop for i from 1 below k
                              when (< (cdr (aref heap i)) min-val)
                              do (setf min-idx i min-val (cdr (aref heap i))))
                        (setf (aref heap min-idx) (cons hash sim))
                        (setf heap-min (loop for i below k
                                             minimize (cdr (aref heap i)))))))))
               (obs-graph-embeddings graph))
      ;; Return sorted descending
      (sort (coerce (subseq heap 0 (fill-pointer heap)) 'list)
            #'> :key #'cdr))))

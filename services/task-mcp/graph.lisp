...
(defun get-or-build-graph ()
  "Build the full task graph from *tasks-root*.
   Returns cached graph with bare task IDs.
          ...
          (with-output-to-string (s)
            (if frontier
                (let ((scored nil))
                  ...
                  (dolist (fid frontier)
                    (handler-case
                        (let* ((state (task:get-cached-task-state fid))
                               (obs-count (if state
                                              (crdt:gs-count (task:task-state-observations state))
                                              0))
                               (sess-count (if state
                                               (crdt:gs-count (task:task-state-sessions state))
                                               0))
                               (claim (when state
...
(define-tool task_health ()
  ...
  (handler-case
      (sb-ext:with-timeout 45
        (make-text-content "~A" (task:task-health-report)))
    (sb-ext:timeout ()
      (make-text-content "Health report timed out (>45s). Try again with warm caches."))))

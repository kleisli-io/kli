...
plan-markdown (generated plan.md), plan-json (plan as JSON for dashboard)."
  (let* ((graph (get-or-build-graph))
         (id (or (when (and task_id (> (length task_id) 0)) task_id)
                 *current-task-id*))
         (task-infos (task:get-cached-task-infos))
         ;; Temporal edges: iterate per depot, passing bare IDs + depot root
         (ts-edges (loop for depot being the hash-keys of task:*depot-tasks-roots*
    ...
    (make-text-content "~A"
     (cond
       ...
       ((string= query "knowledge")
        (let ((reachable (task:reachable-at-time ts-edges (or id "")
                                                 "2026-12-31_23-59-59")))
          (with-output-to-string (s)

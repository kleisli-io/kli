...
(defvar *ollama-failure-count* 0
  "Consecutive ollama failures. Reset on success.")

(defvar *ollama-cooldown-until* 0
  "Universal time until which ollama calls are skipped.")

(defparameter *ollama-max-failures* 3
  "Skip ollama after this many consecutive failures.")

(defparameter *ollama-cooldown-seconds* 60
  "Seconds to skip ollama after tripping circuit breaker.")

(defun ollama-available-p ()
  "Check if ollama should be attempted (circuit breaker not tripped)."
  (or (< *ollama-failure-count* *ollama-max-failures*)
      (> (get-universal-time) *ollama-cooldown-until*)))

(defun ollama-record-success ()
  "Record successful ollama call — reset circuit breaker."
  (setf *ollama-failure-count* 0))

(defun ollama-record-failure ()
  "Record failed ollama call — trip breaker after max failures."
  (incf *ollama-failure-count*)
  (when (>= *ollama-failure-count* *ollama-max-failures*)
    (setf *ollama-cooldown-until*
          (+ (get-universal-time) *ollama-cooldown-seconds*))))

;;; Cached embedding retrieval

(defparameter *embedding-cache-max-size* 10000
  "Maximum number of cached embeddings. Oldest entries evicted when exceeded.")

(defun evict-embedding-cache-if-full ()
  "Remove ~10% of entries when cache exceeds max size. Must be called under lock."
  (when (> (hash-table-count *embedding-cache*) *embedding-cache-max-size*)
    (let ((to-remove (ceiling (* (hash-table-count *embedding-cache*) 0.1)))
          (removed 0))
      (block eviction
        (maphash (lambda (k v)
                   (declare (ignore v))
    ...
    (or cached
        ...
        (when (ollama-available-p)
          (let ((embedding (ollama-embed text)))
            (if embedding
                (progn
                  (ollama-record-success)
                  (bt:with-lock-held (*embedding-cache-lock*)
                    (or (gethash text *embedding-cache*)
                        (progn
                          (evict-embedding-cache-if-full)

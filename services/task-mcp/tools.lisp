...
(defun resolve-depot-for-creation (explicit-depot task-name)
  ...
  (let* ((target-depot (or explicit-depot task:*current-depot* "default"))
    ...
    (when (and explicit-depot
               task:*current-depot*
               (not (string= explicit-depot task:*current-depot*)))
      (format *error-output* "~&Note: Creating task in depot '~A' (current: '~A').~%"
              explicit-depot task:*current-depot*))
...
(defun resolve-task-id (id)
  "Resolve a task ID to a qualified ID.
   ...
   If unqualified, tries to find the task in:
     1. Current depot (*current-depot*)
     2. Any known depot in *depot-tasks-roots*
  ...
  (if (task:qualified-id-p id)
      ...
      (let ((current-depot (or task:*current-depot* "default")))

  ...
  (let ((validation-error (validate-name-or-error name)))
    (or validation-error
        (let* ((full-name (ensure-date-prefix name))
               (dir (task:ensure-task-directory full-name))
          ...
          ;; Bind *in-mutation* to suppress registry reads/writes during
          ;; transient *current-task-id* switch — prevents concurrent
          ;; requests (sub-agents sharing same session) from loading
          ;; intermediate task ID from the shared registry.
          (unwind-protect
               (let ((*in-mutation* t))
            ...
            ;; Outside *in-mutation* scope so registry save is not suppressed.
            (setf *current-task-id* (or prev full-name))
  ...
  (let ((validation-error (validate-name-or-error name)))
    (or validation-error
        (let* ((full-name (ensure-date-prefix name))
          ...
          ;; Fork never switches current task.
          ;; Bind *in-mutation* to suppress registry reads/writes during
          ;; transient *current-task-id* switches — prevents concurrent
          ;; requests (sub-agents sharing same session) from loading
          ;; intermediate child/parent task IDs from the shared registry.
          (unwind-protect
               (let ((*in-mutation* t))
                 ;; Create child task
                 (setf *current-task-id* full-name)
                 (emit-event :task.create
                             (append (list :name full-name
                                           :description description)
                 ...
                 (when parent-id
                   ...
                   (emit-event :task.fork
                               (list :child-id full-name :edge-type etype))))
            ;; Restore context and persist to registry (outside *in-mutation*
            ;; scope so save is not suppressed).
            (setf *current-task-id* prev)
            (when *http-mode* (save-session-context *session-id*)))
          (make-text-content
           (if parent-id
               (format nil "Forked ~A from ~A (edge: ~A) at ~A" full-name parent-id etype dir)
               (format nil "Created top-level task ~A at ~A" full-name dir)))))))
         ...
         (matching (remove-if-not
                    (lambda (sess)
                      (let ((sess-task (getf sess :task-id)))
                        (and sess-task
                             (string= (task:strip-depot-prefix sess-task)
                                      (task:strip-depot-prefix task-id)))))
                    all-sessions)))
    (if matching
...
(defun bootstrap-task (task-id)
  "Bootstrap full context for a task in one call.
   ...
   Two-phase design:
   - Core phase (always runs): task state, graph, neighbors, enriched query, handoff
   - Swarm phase (30s deadline): related sessions, similarity, frontier, orphans
   If the swarm phase exceeds 30s, partial core results are returned with a warning."
  (ensure-session-context)
  (let ((task-id (handler-case (join-task task-id)
                   (error (e)
                     (return-from bootstrap-task (format nil "~A" e))))))
    ;; --- Core phase: always runs, produces essential context ---
    (let* ((warnings nil)
           ...
           (latest-handoff (when handoff-files
                             (namestring
                              (first (sort handoff-files #'string>
                                           :key #'namestring)))))
           ;; --- Swarm phase: 30s deadline, skippable ---
           (swarm-str "")
           (similar-str "")
           (frontier-str "")
           (orphan-str ""))
      (handler-case
          (sb-ext:with-timeout 30
            ...
            (let ((related-sessions
                    (handler-case (find-related-sessions task-id)
                      (error ()
                        (push "Swarm awareness: unavailable" warnings)
                        nil))))
              ;; format-swarm-awareness returns (values string orphans)
              (multiple-value-bind (str orphans)
                  (format-swarm-awareness task-id related-sessions)
                (setf swarm-str str)
                ;; Orphan warnings (reuse orphans from swarm-awareness)
                (setf orphan-str
                      (handler-case
                          (format-orphan-warnings task-id
                            :pre-computed-orphans orphans)
                        (error ()
                          (push "Orphan detection: unavailable" warnings)
                          "")))))
            ;; Behavioral similarity
            (handler-case
                (multiple-value-bind (sessions degraded)
                    (find-similar-sessions *session-id* :max-age-hours 1)
                  ;; Merge vector clocks from similar sessions
                  (handler-case
                      (when sessions
                        (merge-vector-clocks-from-sessions
                         (mapcar (lambda (s) (getf s :session-id)) sessions)))
                    (error () nil))
                  (setf similar-str
                        (format-similar-sessions sessions :degraded degraded)))
              (error ()
                (push "Similar session detection: unavailable" warnings)))
            ;; Swarm-aware plan frontier
            (setf frontier-str
                  (handler-case (format-swarm-frontier task-id *session-id*)
                    (error ()
                      (push "Plan frontier: unavailable" warnings)
                      ""))))
        (sb-ext:timeout ()
          (push "Swarm phase timed out after 30s (core results returned)" warnings)))
      ;; --- Output phase: compose final string ---
      (with-output-to-string (s)
        ...
        ;; Append swarm sections (may be empty strings if timed out)
        (write-string swarm-str s)
        (when (plusp (length similar-str))
          (terpri s)
...
(define-session-tool task_complete
  ...
  (let ((id (or (when (and task_id (> (length task_id) 0))
    ...
    ;; Route the emit explicitly to ID via :task-id.  The session
    ;; registry's ctx-task-id is left untouched, so concurrent requests
    ;; on the same MCP session continue to see the user's focus task as
    ;; their current task — the completion target is invisible to them.
    (emit-event :task.update-status (list :status "completed") :task-id id)
...
(define-session-tool task_reopen
  ...
  (let ((id (or (when (and task_id (> (length task_id) 0))
    ...
    (unless id
      (error "No task specified and no current task set."))
    (emit-event :task.update-status (list :status "active") :task-id id)
    (make-text-content
...
(define-task-tool spawn
  ...
  (let ((validation-error (validate-name-or-error name)))
    (or validation-error
        (let* ((full-name (ensure-date-prefix name))
               (child-dir (task:ensure-task-directory full-name))
               (parent *current-task-id*))
          ;; Bind *in-mutation* to suppress registry reads/writes during
          ;; transient *current-task-id* switches — prevents concurrent
          ;; requests from loading intermediate task IDs.
          (unwind-protect
               (let ((*in-mutation* t))
                 ;; Create child task
                 (setf *current-task-id* full-name)
                 ...
                 (emit-event :task.fork
                             (list :child-id full-name :edge-type "phase-of")))
            ;; Restore context and persist to registry (outside *in-mutation*).
            (setf *current-task-id* parent)
            (when *http-mode* (save-session-context *session-id*)))
          (make-text-content
           (format nil "Spawned child task ~A at ~A" full-name child-dir))))))
...
(define-session-tool task_release
  ...
  (let ((id (or (when (and task_id (> (length task_id) 0))
                ...
                *current-task-id*)))
    (unless id (error "No task specified and no current task set."))
    (emit-event :session.release nil :task-id id)
                        ...
                        (let* ((cstate (gethash c states-ht))
                               (cstatus (if cstate
                                            (or (crdt:lww-value
                                                ...
                                                "?")
                                            "?"))
                               (short-name (task:strip-depot-prefix c)))
                          (format s "  ~A ~A~%"
                          ...
                          (format s "  [~A] ~A~%"
                                  ...
                                  (if (> (length text) 120)
                                      (format nil "~A..."
                                              (subseq text 0 117))
                                      text)))))))
                ;; Health available via task_health tool (not included here for speed)
                ))))))))

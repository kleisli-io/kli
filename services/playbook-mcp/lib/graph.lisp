...
(defun rebuild-graph (&key manual-edges-path)
  "Rebuild the graph from all edge sources. Atomic swap.
   ...
   4. Ensure all patterns have embeddings (for proximity calculation)
   5. Compute proximity edges from pattern embeddings
   6. Build new store and atomic swap
   Returns a status string."
  ;; Ensure embeddings exist (needed for semantic search even without proximity edges)
  (compute-all-embeddings)
  ;; NOTE: Proximity edges disabled — with nomic-embed-text, 85% of pattern pairs
  ;; exceed threshold 0.5, producing a near-complete graph that drowns signal.
  ;; Co-app + manual edges carry the meaningful structural signal.

...
(defun meta-root-for (&optional depot)
  "Get the metadata directory (.kli/) from *tasks-root*."
  (declare (ignore depot))
  (when task:*tasks-root*
    (truename (merge-pathnames "../" (pathname task:*tasks-root*)))))

(defun depot-root-for (&optional depot)
  "Get the repository root directory (parent of .kli/)."
  (declare (ignore depot))
  (let ((meta (meta-root-for)))
    (when meta
...
(defun scratchpad-file-path (task-id)
  "Get scratchpad file path. Handles __global__ sentinel for global scratchpad."
  (let* ((bare-id (task:strip-depot-prefix task-id))
         (meta (meta-root-for)))
    (when bare-id
      (if (string= bare-id "__global__")
          ...
          (when task:*tasks-root*
            (merge-pathnames (format nil "~A/.scratchpad.md" bare-id)
                             task:*tasks-root*))))))
...
(defun task-detail-data (task-id)
  ...
  (with-safe-data nil
    (let* ((id task-id)
           ...
           ;; Graph neighbors
           (graph (ignore-errors (task:get-cached-task-graph)))
           (bare-id (task:strip-depot-prefix id))
           (fwd (when graph (gethash bare-id (task:task-graph-forward graph))))
           (rev (when graph (gethash bare-id (task:task-graph-reverse graph))))
           (neighbors (append
    ...
    (lambda ()
      (with-safe-data nil
        (let* ((all-tasks (if depot
               ...
               (graph (task:get-cached-task-graph))

...
tools: Grep, Glob, Read, LS, Search, mcp__depot_intel__depot_search
model: sonnet
contract:
  input:
    format: json
    schema:
      ...
      properties:
        pattern_description:
          type: string
          description: What pattern to find (code pattern, naming convention, architectural approach)
        scope:
          type: string
...
- You can ONLY use these tools: Grep, Glob, Read, LS, Search, mcp__depot_intel__depot_search

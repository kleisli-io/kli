...
allowed-tools: mcp__task__*, mcp__playbook__*, Read, Write, Edit, Task, Bash, Glob, Grep
---
...
**0b. Load relevant skills**: Determine what kind of implementation this is. If design-related, load the frontend-design and automating-browsers skills.
...
2. **Record novel insights as observations** — workarounds, anti-patterns, techniques used 2+ times:
   ...
   ```
3. **Do NOT use `(add! ...)`** — pattern promotion goes through `/ace:reflect` (Reflector → Curator)

Present completion summary with next steps (`/ace:validate`, `/core:commit`, `/ace:reflect`).

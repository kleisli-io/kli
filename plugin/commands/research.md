---
description: Document codebase as-is through iterative research with observation capture (includes visual design research)
allowed-tools: mcp__task__*, mcp__playbook__*, Task, Read, Glob, Grep
---

# Research Codebase

Research the codebase or visual design patterns using specialized agents.
...
Five research strategies are available:

| Strategy | Keywords | Agents |
|----------|----------|--------|
| **codebase** | "how", "where", "implementation", "code", "architecture" | codebase-locator, codebase-analyzer, pattern-finder |
| **visual** | "design", "UI", "visual", "component", "inspiration", "peer" | visual-design-researcher, visual-code-extractor |
| **github** | "repo", "repository", "github.com", "open source", "package source" | github-researcher |
| **external** | "library docs", "framework", "documentation", "how to use X" | web-search-researcher |
| **graph** | "prior tasks", "patterns for", "project health", "task history", "related tasks", "what has been done" | graph-analyst |
...
```
Task(
    subagent_type="visual-design-researcher",
    prompt="<research question>. Task dir: <task_dir>",
    description="Research visual design patterns"
)
```
...
Task(
    ...
    prompt="INPUT[1]{task_dir,repo,query_type,focus}:\n <task_dir>,<owner/repo>,structure,",
    description="Analyze GitHub repository"
)
```

Use when query contains GitHub URLs (github.com/...) or asks about open source repositories.

**For external documentation/library research:**

```
Task(
    subagent_type="web-search-researcher",
    prompt="INPUT[1]{task_dir,query,query_type}:\n <task_dir>,<query>,library",
    description="Research external documentation"
)
```

Use for library/framework questions that need current documentation from the web.
...
- Start with `codebase-locator` for "where is X?" questions (local code)
- Use `codebase-analyzer` for "how does X work?" questions (local code)
- Use `pattern-finder` for "how is X done elsewhere?" questions (local code)
- Use `visual-design-researcher` for design inspiration and trends
- Use `github-researcher` for analyzing external GitHub repositories
- Use `web-search-researcher` for library docs and external knowledge
- Use `graph-analyst` for prior tasks, pattern effectiveness, project health, task relationships
...
1. First spawn codebase agents to understand current implementation
2. Then spawn `visual-design-researcher` to gather design patterns
...
**Do NOT use `(add! ...)` during research.** Research follows the documentarian philosophy — document what IS, not prescribe what to DO. Findings are observations by nature. Reusable patterns emerge later during reflection (`/ace:reflect`), which applies the litmus test:
...
## Visual Research Specifics

The visual-design-researcher agent applies pattern [ace-127]:
- 5-8 distinct categories (peers, aspirational, cultural, domain leaders)
- 4-6 exemplars per category (40+ total sites)
- Consensus thresholds: 87%+ = safe default, 60-86% = document variance

After visual research completes, ask:
```
Would you like me to extract code from any of the discovered examples?
```

If yes, spawn `visual-code-extractor` with URL and target selector:
```
Task(
    subagent_type="visual-code-extractor",
    prompt="Extract component from <URL>. Target: <selector>",
    description="Extract component code"
)
```
...
- **Record findings** as observations via `observe()` — patterns are promoted during `/ace:reflect`

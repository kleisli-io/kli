...
allowed-tools: mcp__task__*, mcp__playbook__*, Read, Write, Task, AskUserQuestion
---

# Implementation Plan

Create detailed, phased implementation plans as task DAGs following ACE methodology.
...
- Token savings: Reusing research.md saves 40-50% tokens vs spawning duplicate sub-agents (pattern [ace-078])
...
Record planning insights as observations (patterns are promoted during `/ace:reflect`):
```
...
Next step: `/ace:implement` to execute the phases.
...
- **Reuse research.md** if available (saves 40-50% tokens via [ace-078])
...
- `ace/lib/task-model.md` - Task system reference (plans as DAGs, graph queries)

...
Scaffold an event-sourced task with the task MCP server. This is for quick, focused work that doesn't need the full ACE research/plan/implement cycle.
...
When done, mark complete with task_complete() or hand off with /ace:handoff.
```
...
| Error | Response |
|-------|----------|
| Task MCP unavailable | "Task MCP server not responding. Are you in a depot shell with task tracking enabled?" |

...
allowed-tools: mcp__task__*, mcp__playbook__*, Read, Glob, Task
---

# Resume Task

Resume work on a task by gathering context from the task MCP server's event stream, graph state, and artifacts. Unlike `/ace:resume_handoff` which requires a handoff document, this command works directly with the task's live state.
...
Use `/ace:resume_handoff` instead when a handoff document exists and you want to follow its specific guidance.
...
1. Start researching this task (/ace:research)
2. Create a plan (/ace:plan)
...
Would you like me to:
1. Resume from the latest handoff (/ace:resume_handoff)
...
3. Running /ace:validate if implementation was in progress
```
...
[Task Created]
     ↓
/ace:research → /ace:plan → /ace:implement → /ace:validate → /ace:reflect
     ↑                                              ↓
     └──────────── /core:resume-task ←─────────────┘
                  (re-enter anywhere)
```

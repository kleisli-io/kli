...
<!-- ACE Metadata:
  source: ace/playbooks/ace.md, .claude/commands/research.md
  command-complement: .claude/commands/research.md
  sync-threshold: 3
  last-synced: 2026-01-02
  version: 1.1
  pattern-ids: [ace-001], [ace-007], [ace-009], [ace-036], [ace-037], [ace-078]
-->

# ACE Research Phase Skill
...
**Execute Generator Role in ACE Workflow**:
...
3. **Spawn parallel agents** in single message
   ...
   - RARELY: web-search-researcher (external docs)
...
3. Patterns are promoted during `/ace:reflect` via the litmus test (transferable? actionable? prescriptive?)
...
**web-search-researcher** (EXTERNAL TEXT questions):
- **When**: Need external documentation, technical articles, best practices
- **Why**: Supplement codebase knowledge with text-based information
- **What to tell it**: What to research online (non-visual topics), request LINKS
- **What it knows**: WebSearch, WebFetch, Context7 for library/framework docs
- **Default**: Prefer codebase-only unless clear external need
- **Scope**: Text-based research (documentation, articles, concepts)

**visual-research-agent** (VISUAL/DESIGN questions):
- **When**: Need UI/design patterns, component examples, visual inspiration
- **Why**: Gather visual evidence (screenshots, images, design system analysis)
- **What to tell it**: What visual patterns to find, which sites to analyze
- **What it knows**: Firecrawl (screenshots, branding, images), Context7 (UI frameworks)
- **Use cases**:
  - Component examples with screenshots ("Find modern card designs")
  - Design system analysis ("Analyze Stripe's design system")
  - Visual inspiration gathering ("Modern dashboard layouts")
  - Branding research (colors, typography, spacing extraction)
  - ERCOM peer institution analysis (visual comparison)
- **Output**: Visual findings with screenshots, design system breakdowns, branding data

**Research Type Detection**:

**Visual/Design Research Indicators** → Use visual-research-agent:
...
**Text/Documentation Research** → Use web-search-researcher:
...
**Mixed Research** → Spawn both in parallel:
- UI framework research (Tailwind, React, Vue components)
- Design + implementation patterns
- Spawn visual-research-agent for visual examples
- Spawn web-search-researcher for technical docs
- Synthesize both findings in research.md

**Spawning Strategy** (Pattern [ace-036] - parallel spawning):
```
...
Example (visual design research):
Message 1: visual-research-agent (parallel with playbook patterns)
→ visual-research-agent gathers component examples with screenshots

Example (mixed UI framework research):
Message 1: visual-research-agent + web-search-researcher (parallel)
→ visual-research-agent: Component examples, screenshots
→ web-search-researcher: Framework documentation, best practices
```
...
- External library used, but need web-researcher for docs

### Artifact Reuse Pattern (Pattern [ace-078])
...
2. If approved → Run `/plan` to create implementation plan (reuses research.md)

...
<!-- ACE Metadata:
  source: ace/playbooks/ace.md, .claude/commands/plan.md
  command-complement: .claude/commands/plan.md
  sync-threshold: 3
  last-synced: 2026-01-02
  version: 1.1
  pattern-ids: [ace-014], [ace-017], [ace-052], [ace-060], [ace-078], [ace-111]
-->

# ACE Planning Phase Skill
...
2. **Testable**: Clear verification points
   - NOT: "Update code"
   - YES: "Update code (verified by: nix build tests pass)"
...
**Both Required** (Pattern [ace-052] - never proceed without verification):

**Automated Verification:**
- Build success: `nix build -f . <target>`
- Tests pass: `nix build -f . <tests-target>`
- Syntax checks: `nix-instantiate --parse`, `sbcl --load`
...
The ACE planning phase transforms research findings into detailed, phased implementation plans. It reuses research artifacts (saves 40-50% tokens), defines clear phase boundaries with verification gates, and incorporates playbook patterns to ensure implementation success.

**Role in ACE Workflow:**
...
3. **Artifact Consumption**: Detects and reuses `research.md` ([ace-078])
...
1. **Automated Verification**:
   ```markdown
   **Automated Verification:**
   - [ ] Build passes: `nix build -f . target`
   - [ ] Tests pass: `nix build -f . target.tests`
   - [ ] Syntax valid: `nix-instantiate --parse file.nix`
...
### Research Artifact Reuse (Pattern [ace-078])
...
4. Handle user choice:
   ...
   - Option 3: Offer to run /research instead
```
...
- Consider suggesting user run /research first if complex
...
- **[ace-014]** helpful=5: Create detailed plan before implementation (not rough outline)
...
- **[ace-060]** helpful=22: Don't batch multiple phases - implement incrementally with verification
- **[ace-078]** helpful=8: Commands detect and reuse research.md (saves 40-50% tokens)
- **[ace-111]** helpful=3: Phase scope reduction is valid mid-implementation when earlier phases deliver sufficient user value - strategic prioritization, not scope creep
...
- [ ] Configs load: `nix-instantiate file.nix`
- [ ] No syntax errors
...
**Command Source:**
- `.claude/commands/plan.md:70-129` - Research artifact reuse pattern
- `.claude/commands/plan.md:314-319` - Phase design principles
- `.claude/commands/plan.md:370-383` - Clarifying questions template
- `.claude/commands/plan.md:398-410` - Exit criteria definition
...
4. **Record planning insights as observations** (patterns promoted during `/ace:reflect`):
   ```
   observe("Planning insight: <description>. Evidence: plan.md approach")
   ```
...
2. User reviews plan.md for completeness
3. If approved → Run `/ace:implement` to execute plan phase-by-phase
4. If changes needed → Iterate on plan (update plan.md)
5. Implementation phase will load plan.md and execute phases sequentially
...
- `ace/lib/task-model.md` - Task system reference (plans as DAGs, TQ mutations)
